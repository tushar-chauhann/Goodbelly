...
const Consultations = () => {
  ...
  const renderConsultantCard = ({ item }) => {
    ...
    return (
      <View
        style={tw`bg-white rounded-2xl p-4 mb-4 shadow-sm border border-gray-200 flex-row`}
      >
        {/* Consultant Image */}
    ...
    <SafeAreaView style={tw`flex-1 bg-white`}>
      ...
      <View style={tw`bg-gray-50 mx-4 rounded-xl p-4 mb-8`}>
        ...
        <View style={tw`flex-row`}>
          <TouchableOpacity
            style={tw`border border-[#6A8B78] rounded-lg px-4 py-2 mr-2 flex-1`}
            onPress={() => navigation.navigate("ConsultantLogin")}
          >
            <Text style={[fontStyles.headingS, tw`text-[#6A8B78] text-center text-sm`]}>
              Consultant login
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
