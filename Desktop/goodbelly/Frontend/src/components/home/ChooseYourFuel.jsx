...
const ChooseYourFuel = () => {
  ...
  const fetchCategories = async () => {
    try {
      ...
      // 1. Try Occasions (Primary)
      try {
        const occasionsResponse = await authService.getOccasions();
        if (occasionsResponse?.data && Array.isArray(occasionsResponse.data) && occasionsResponse.data.length > 0) {
          list = occasionsResponse.data;
        }
      } catch (err) {
        console.log("Failed to fetch occasions, trying categories...");
      }

      // 2. If no occasions, try Categories (Secondary)
      if (list.length === 0) {
        try {
          const categoriesResponse = await authService.getCategories();
          ...
          console.log("Failed to fetch categories too.");
        }
      }

      if (list.length === 0) {
        console.log("    Using fallback fuel categories");
        list = [
      ...
      console.error("ChooseYourFuel Error:", e);
