...
const SeeMoreButton = () => {
  ...
  const fetchCatsAndOccs = useCallback(async () => {
    try {
      ...
      if (cachedData) {
        ...
        if (cachedData.length && !selectedToken) {
          ...
          else if (selectedCategory && !hasHandledInitialSelection.current) {
  ...
  const fetchFreshCatsAndOccs = async () => {
    try {
      ...
      if (!leftList.length) {
        ...
        if (combined.length && !selectedToken) {
          // Priority 1: If we have ingredientId from route, don't auto-select category
          if (ingredientId) {
            hasHandledInitialSelection.current = true;
          }
  ...
  const applyFiltersToProducts = useCallback(() => {
    ...
    if (dietaryPreference) {
      filteredList = filteredList.filter(
        (product) => product.productType === dietaryPreference
      );
    }
    ...
    setProducts(sorted);
    setVisibleCount(INITIAL_COUNT); // Reset visible count when filters change
  }, [allProducts, dietaryPreference, nutriBand, sortBy]);

  useEffect(() => {
    fetchCatsAndOccs();
  }, [fetchCatsAndOccs]);

  //   ADD: Refresh authentication state when component mounts
  useEffect(() => {
  ...
  // FIXED: Only handle selectedCategory on initial load
  useEffect(() => {
    if (
      selectedCategory &&
      leftList.length > 0 &&
      !hasHandledInitialSelection.current &&
      !ingredientId // Don't override if we have ingredientId
    ) {
      const matchedCategory = leftList.find((item) => {
        const itemKey = item.id.replace(/^(cat:|occ:)/, "");
        return (
          itemKey === selectedCategory ||
          item.name.toLowerCase().includes(selectedCategory.toLowerCase())
      ...
      if (matchedCategory) {
        setSelectedToken(matchedCategory.id);
        hasHandledInitialSelection.current = true;
        //   REMOVED: Auto-scroll to prevent interrupting user browsing
        // setTimeout(() => {
        //   scrollToSelectedItem();
        // }, 600);
      }
    }
  }, [selectedCategory, leftList, ingredientId]);
  ...
  //   FIXED: Clear all filters like web version
  ...
  //   FIXED: Apply filters function - now only updates state, filtering happens in useEffect
  const handleApplyFilters = (filters) => {
    setDietaryPreference(filters.dietaryPreference);
    setNutriBand(filters.macroFilters);
    setFilterOpen(false);
    // No need to manually fetch products - useEffect will handle filtering
  };
  ...
  const ViewMoreCard = ({ onPress }) => (
    <TouchableOpacity
      activeOpacity={0.9}
      onPress={onPress}
      style={tw`w-full my-2 items-center`}
    >
      <Text
        style={[fontStyles.bodyBold, tw`text-[13px] font-bold text-gray-800`]}
      >
        View More
      </Text>
    </TouchableOpacity>
  ...
  const handleViewMore = () =>
    setVisibleCount((prev) => Math.min(prev + STEP_COUNT, products.length));
    ...
    <SafeAreaView style={tw`flex-1 bg-white`}>
      ...
      <View style={tw`flex-1 flex-row`}>
        ...
        <View style={tw`flex-1`}>
          ...
          >
            <View style={tw`flex-row items-center gap-1 my-3`}>
              <SortButton />
              <FilterButton />
            </View>
            ...
            {loadingProds ? (
              ...
              <View style={tw`mx-0`}>
                <FlatList
                  data={
                    hasMore
                      ? [...sliced, { id: "__viewmore__", type: "viewmore" }]
                      : sliced
                  }
                  keyExtractor={(item) =>
                    String(
                      item.id ?? item.detailId ?? item.sku ?? Math.random()
                    )
                  }
                  numColumns={2}
                  ...
                  extraData={[
                    ...
                    favById,
                    favLoadingById,
                    cartItems, //   FIXED: Include cartItems for proper re-rendering
                    cart, //   ADD: Include entire cart object
                  ]}
                />
              </View>
            )}
