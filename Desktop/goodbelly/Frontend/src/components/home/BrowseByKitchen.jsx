...
import ProductSkeleton, { CategorySkeleton, KitchenProductSkeleton } from "../ProductSkeleton";
...
const BrowseByKitchen = ({ refreshTrigger }) => {
  ...
  // Fetch kitchens and all products
  const fetchData = async (showLoading = true) => {
    try {
      if (showLoading) setLoading(true);
      setError(null);

      const [kitchensResponse, productsResponse, wishlistResponse] = await Promise.all([
        authService.getKitchens(),
      ...
      // Handle kitchens response - try different data structures
      let kitchensData = [];
      ...
      // If no kitchens found, extract from products
      if (kitchensData.length === 0 && productsResponse) {
        let productsData = [];

        // Handle different products response structures
        ...
        // Initialize favorites from fetched products
        const newFavs = {};
        ...
        kitchensData = Array.from(vendorsMap.values());
      }
      ...
      if (productsResponse) {
        ...
        // Filter products to only include verified ones with images
  ...
  const fetchVendorProducts = async (vendorId) => {
    try {
      ...
      const response = await authService.getVendorProducts(vendorId);
      ...
      // Update favorites state with new products
      setFavById((prev) => {
        const next = { ...prev };
        validProducts.forEach((p) => {
          if (p.id && next[p.id] === undefined) {
            next[p.id] = !!p.isFavorite;
          }
        });
        return next;
      });
      ...
      // If vendor products fail, filter from all products
      const vendorProductsFromAll = products.filter(
        (product) =>
          product.vendor &&
          (product.vendor.id === vendorId || product.vendor._id === vendorId)
      );
      setVendorProducts(vendorProductsFromAll);
    } finally {
  ...
  useEffect(() => {
    if (selectedKitchen) {
      fetchVendorProducts(selectedKitchen);
    }
          ...
          <View style={tw`flex-1`}>
            ...
            <View style={tw`flex-row justify-between items-center mt-auto`}>
              ...
              <View style={tw`items-end`}>
                {currentQuantity === 0 ? (
                  <TouchableOpacity
                    style={tw`bg-[#6B9080] w-20 h-8 justify-center items-center rounded-xl shadow-sm`}
                    onPress={() => handleAddToCart(productId, weightId, product)}
                  >
                    <Text style={[fontStyles.bodyBold, tw`text-white text-xs`]}>Add</Text>
                  </TouchableOpacity>
                ) : (
                  <View style={tw`flex-row items-center justify-between bg-[#6B9080] rounded-xl w-20 h-8 px-2 shadow-sm`}>
                    <TouchableOpacity
                      style={tw`w-6 h-full justify-center items-center`}
                      onPress={() => {
                    ...
                    <Text style={[fontStyles.bodyBold, tw`text-white text-xs mx-1`]}>
                      {currentQuantity}
                    </Text>
    ...
    return (
      <View style={tw`px-4 mt-2`}>
        ...
        <View style={tw`mx-0`}>
          <FlatList
            data={[1, 2, 3, 4]}
            renderItem={() => <KitchenProductSkeleton />}
