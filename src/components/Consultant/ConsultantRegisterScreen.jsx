...
export default function ConsultantRegisterScreen({ navigation }) {
    ...
    const getResponsiveSizes = () => {
        if (width >= 768) return { stepCircleSize: 48, stepTitleWidth: 100, fontSize: { body: 15, small: 13 } };
        if (width >= 414) return { stepCircleSize: 44, stepTitleWidth: 95, fontSize: { body: 14, small: 12 } };
        if (width >= 375) return { stepCircleSize: 44, stepTitleWidth: 90, fontSize: { body: 14, small: 12 } };
        return { stepCircleSize: 40, stepTitleWidth: 80, fontSize: { body: 13, small: 11 } };
    };
                                ...
                                {REGISTRATION_STEPS.map((step, index) => {
                                    ...
                                    return (
                                        <View key={step.id} style={tw`items-center ${isLast ? 'mr-0' : 'mr-5'}`}>
                                            ...
                                            <View
                                                style={[
                                                    tw`rounded-full items-center justify-center mb-2`,
                                                    {
                                                        width: sizes.stepCircleSize,
                                                        height: sizes.stepCircleSize,
                                                        backgroundColor: isCompleted || isCurrent ? PRIMARY_COLOR : "#E5E7EB",
                                                    },
                                                ]}
                                            >
                                                {isCompleted ? (
                                                    <Ionicons name="checkmark" size={sizes.stepCircleSize * 0.4} color="white" />
                                                ) : (
                                                    <Text
                                                        style={[
                                                            fontStyles.body,
                                                            { fontSize: sizes.fontSize.body, fontWeight: 'bold' },
                                            ...
                                            <Text
                                                style={[
                                                    fontStyles.body,
                                                    {
                                                        fontSize: sizes.fontSize.small - 1,
                                                        color: '#9CA3AF',
                                                        textAlign: 'center',
                                                        lineHeight: 14,
                                                        width: sizes.stepTitleWidth,
