...
const ConsultWithExpert = () => {
  ...
  // Get responsive badge sizes based on screen width
  const getBadgeSizes = () => {
    if (width >= 1024) {
      return { fontSize: 10, iconSize: 12, px: 2, py: 1 };
    } else if (width >= 768) {
      return { fontSize: 9, iconSize: 11, px: 2, py: 0.75 };
    } else if (width >= 414) {
      return { fontSize: 8, iconSize: 10, px: 1.5, py: 0.5 };
    } else {
      return { fontSize: 8, iconSize: 10, px: 1.5, py: 0.5 };
    }
  };

  const badgeSizes = getBadgeSizes();

  // Fetch consultants from API with top experts prioritization
  const fetchConsultants = async () => {
    try {
      setLoading(true);
      setError(null);

      // Fetch both regular and top consultants in parallel
      ...
      console.log("  Consultants fetched:", {
        total: all.length,
        topExperts: top.length,
      });

      // Remove top consultants from regular list to avoid duplicates
      const nonPinned = all.filter(c => !top.some(t => t.id === c.id));

      // Merge: top experts first, then others
      const merged = [...top, ...nonPinned];

      // Take only first 4 consultants for display
      setExperts(merged.slice(0, 4));
      setTopConsultants(top);
    } catch (err) {
  ...
  const renderExpert = ({ item }) => {
    ...
    return (
      <View style={[tw`mb-3`, { width: cardWidth }]}>
        <TouchableOpacity
          style={[
            tw`bg-white rounded-xl p-3 flex-row border border-gray-200 min-h-32 relative`,
        ...
        >
          ...
          {item.isPinned && (
            <View
              style={[
                tw`absolute top-18 left-6.1 z-10 bg-[#6B9080] rounded-full flex-row items-center shadow-sm`,
                {
                  paddingHorizontal: badgeSizes.px * 4,
                  paddingVertical: badgeSizes.py * 4,
                },
              ]}
            >
              <Ionicons name="checkmark-circle" size={badgeSizes.iconSize} color="white" />
              <Text
                numberOfLines={1}
                style={[
                  fontStyles.bodyBold,
