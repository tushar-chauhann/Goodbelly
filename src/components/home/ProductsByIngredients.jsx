...
const ProductsByIngredients = ({ onSeeMore, navigation: propNavigation, refreshTrigger }) => {
  ...
  // Ingredient names that should be hidden for veg users
  const NON_VEG_INGREDIENT_NAMES = [
    'egg', 'eggs', 'chicken', 'mutton', 'fish', 'prawn', 'prawns',
    'shrimp', 'meat', 'beef', 'pork', 'lamb', 'seafood', 'tuna', 'salmon',
  ];

  const isVegUser = user?.preference?.toLowerCase() === 'veg';

  // Filter ingredient category bubbles based on user preference
  const visibleIngredientCategories = isVegUser
    ? ingredientCategories.filter(
      (cat) => !NON_VEG_INGREDIENT_NAMES.includes(cat.name?.toLowerCase())
    )
    : ingredientCategories;

  // OPTIMIZED: Removed hydrateUser() (HomeScreen handles it)
  // REDUX-ONLY: Data is managed via Redux thunks
  ...
  // OPTIMIZED: Use Redux products when available — show IMMEDIATELY
  useEffect(() => {
    if (reduxProducts.length > 0) {
      ...
      // Products are ready — stop showing skeleton for 'All' view
  ...
  // OPTIMIZED: Sync ingredients from Redux store for instant rendering
  ...
  useEffect(() => {
    if (reduxIngredients && reduxIngredients.length > 0) {
      const mappedIngredients = reduxIngredients.map((ingredient) => ({
        ...ingredient,
  ...
  // OPTIMIZED: Removed local fetchIngredients (now handled by Redux)
  useFocusEffect(
    React.useCallback(() => {
      // Refresh logic if needed, but normally Redux handles this
  ...
  // Redux-based logic handles refreshing via its own lifecycle
  // and pre-fetching from AppNavigator

  // If selected category becomes hidden (user switched to Veg), reset to "All"
  useEffect(() => {
    if (isVegUser && selectedCategory !== 'All') {
      const isHidden = NON_VEG_INGREDIENT_NAMES.includes(selectedCategory.toLowerCase());
      if (isHidden) setSelectedCategory('All');
    }
  }, [isVegUser, selectedCategory]);
    ...
    if (selectedCategory === "All") {
      products = productsData
        .filter(
          (product) =>
            ...
            (!isVegUser || product.productType === 'VEG')
        )
        .slice(0, 4)
        .map((product) => {
      ...
      if (selectedIngredient && selectedIngredient.products) {
        ...
        const filteredByIngredient = productsData.filter((product) => {
          ...
          const passesVegFilter = !isVegUser || product.productType === 'VEG';

          return ingredientProductIds.includes(product.id) && passesVegFilter;
        });
  ...
  return (
    <View style={tw`px-4`}>
        ...
        <TouchableOpacity
          onPress={() =>
            navigation.navigate("SeeMoreButton", {
              ...
              selectedCategory,
              ingredientFilter: selectedCategory !== "All" ? selectedCategory : null,
            })
          }
      ...
      {categoriesLoading ? (
        ...
        <FlatList
          data={visibleIngredientCategories}
