...
const SmartPeopleSection = () => {
  ...
  // OPTIMIZED: Read products from centralized Redux store
  ...
  // OPTIMIZED: When Redux products are available, use them directly (no API call)
  useEffect(() => {
    if (reduxProducts.length > 0) {
      processProducts(reduxProducts);
    } else {
      fetchAllProductsOnce();
    }
  }, [reduxProducts, user]);

  // OPTIMIZED: Sync wishlist from Redux when available
  useEffect(() => {
  ...
  // Process/split products locally (no API call)
  const processProducts = (allProducts) => {
    const mealsFiltered = sortByPrice(
      allProducts.filter(item => getFinalPrice(item) >= 99)
    );
    const everythingFiltered = sortByPrice(
      allProducts.filter(item => getFinalPrice(item) < 99)
    );

    // Build favs from product data + Redux wishlist
    const newFavs = {};
    ...
    setMealsProducts(mealsFiltered);
    setEverythingProducts(everythingFiltered);
  ...
  // Helper to get final price for a product
  const getFinalPrice = (item) => {
    ...
    return hasDiscount ? discountPrice : rawPrice;
  };
  ...
  // Fallback: only used when Redux has no products yet
  const fetchAllProductsOnce = async () => {
    try {
      setMealsLoading(true);
      setEverythingLoading(true);
      setMealsVisibleCount(10);
      setEverythingVisibleCount(10);
      ...
      processWishlist(wishlistResponse, allProducts);
