...
export default function SearchScreen({ navigation, route }) {
  ...
  const onSpeechError = (e) => {
    ...
    if (errorCode === "11") {
      // Error 11: Didn't understand
      Alert.alert(
        "Didn't catch that",
        "Please try speaking again more clearly.",
        [{ text: "OK" }]
      );
    } else if (errorCode === "7") {
      // Error 7: No match
      Alert.alert(
        "No speech detected",
        "Please try again and speak after tapping the mic.",
        [{ text: "OK" }]
      );
    }
  };
  ...
  const startListening = async () => {
    try {
      ...
      // Clean up any previous sessions before starting
      try {
        await Voice.cancel();
        await Voice.destroy();
      } catch (cleanupError) {
        console.log("Voice cleanup (expected on first run):", cleanupError.message);
      }
  ...
  const stopListening = async () => {
    try {
      await Voice.cancel(); // Use cancel instead of stop for better cleanup
      setIsListening(false);
      setMicStatus("idle");
      setMicColor("#333");
    } catch (error) {
