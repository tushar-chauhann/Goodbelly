...
const AboutUsScreen = () => {
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      //     Debug: Show logged-in user email
      const userStr = await AsyncStorage.getItem("user");
      if (userStr) {
        const user = JSON.parse(userStr);
        console.log("   Your registered email is:", user.email);
        console.log("   You are subscribing with:", email);
        if (user.email !== email) {
          console.warn(" EMAILS DO NOT MATCH! Notification won't be sent.");
        }
      } else {
        console.log("Not logged in. Notification won't be sent.");
      }
  ...
  const sections = [
    {
      title: "Our Mission",
      content:
        "GoodBelly is your holistic wellness partner. We bridge the gap between you and quality home-style kitchens while providing access to expert nutritionists, ensuring every meal supports your health goals.",
    },
    {
      title: "Our Story",
      content:
        "GoodBelly started with a simple idea: make healthy eating personalized and accessible. It has grown into a holistic platform connecting customers with authenticated kitchens and certified experts.",
    },
    {
      title: "What We Offer",
      items: [
        "Verified Cloud Kitchens: Access to a wide network of trusted local kitchens",
        "Smart Subscriptions: Highly customizable meal plans with flexible scheduling",
        "Customizable meal plans for dietary needs",
        "Ingredient-First: Shop by ingredients & curated health collections",
        "Community Driven: Discover top-rated meals through community picks",
      ],
    },
    {
      title: "Our Values",
      items: [
        "Quality First: Rigorous standards for all our kitchen partners",
        "Transparency: Clear ingredient sourcing and nutritional info",
        "Flexibility: Meal plans that adapt to your lifestyle, not vice versa",
        "Holistic Wellness: Combining good food with expert guidance",
        "Customer Focus: Dedicated support for your health journey",
      ],
    },
  ];
