...
import { hydrateUser, setUser } from "../../redux/slicer";
import CustomPopup from "../../components/CustomPopup";
import ConsultantProfileView from "../../components/Consultant/Profile/ConsultantProfileView";
...
const ProfileScreen = () => {
  ...
  const { user: reduxUser, isAuthenticated, status: authStatus } = useSelector((state) => state.auth);
  ...
  const menuItems = [
    ...
    { id: "2b", title: "Macro Tracking", icon: "nutrition-outline", screen: "MacroTracking" },
  ...
  const refreshProfile = async () => {
    // No loader for profile

    try {
      // 1. Refresh auth/user via Redux thunk (handles storage + API)
      const action = await dispatch(hydrateUser());

      // 2. Check for consultant specific profile if needed
      // If the user isn't found or we need extra consultant info
      try {
        const consultantResponse = await authService.getConsultantProfile();
        if (consultantResponse?.data) {
  ...
  const performLogout = async () => {
    try {
      setLogoutLoading(true); await authService.logout();
      // Clear both user and consultant data
      await AsyncStorage.multiRemove(["accessToken", "user", "consultant"]);
  ...
  const displayUser = localUser || reduxUser || {
    name: "Guest User",
  ...
  // Conditional rendering: Show ConsultantProfileView if consultant is logged in
  if (consultant) {
    return (
  ...
  const renderHeader = () => (
    <View style={[tw`bg-[#90a79b]`, { paddingTop: insets.top }]}>
      ...
      <View style={tw`bg-white px-4 pt-4 pb-1`}>
        <Text
          style={[
            fontStyles.headingItalic,
