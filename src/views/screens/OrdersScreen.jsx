...
import { OrderCardSkeleton } from "../../components/ProductSkeleton";
...
const OrderCard = React.memo(({ order, onPress, formatDateTime }) => {
  const isCancelled =
...
export default function OrdersScreen({ navigation }) {
  ...
  useFocusEffect(
    useCallback(() => {
      // Background refresh only if already loaded to prevent flickering
      if (!loading) {
        fetchOrders();
      }
    }, [loading, fetchOrders])
  ...
  const renderOrderItem = useCallback(({ item }) => (
    ...
    />
  ), [handleOrderPress, formatDateTime]);

  if (loading && orders.length === 0) return <LoadingState onBack={handleBack} />;
  if (!loading && orders.length === 0) return <EmptyState onBack={handleBack} />;
  ...
  return (
    <SafeAreaView style={tw`flex-1 bg-gray-50`} edges={["top", "left", "right"]}>
      ...
      <FlatList
        data={orders}
        keyExtractor={(item, index) => item.id || item._id || item.referenceId || String(index)}
