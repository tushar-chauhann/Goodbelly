...
export default function PayUWebViewScreen({ route, navigation }) {
  ...
  // Canonical amount for PayU: prefer passed amount (which matched Hash) over orderData
  const rawAmount = Number(
    payableAmount || amount || paymentParams?.amount || (orderData && (orderData.grandTotal ?? orderData.total ?? orderData.totalAmount)) || 0
  );
...
<body>
  ...
  const navigateToOrderStatus = (statusHint = null) => {
    ...
    // Navigate to HomeScreen as requested by user to avoid OrderStatusScreen errors
    setTimeout(() => {
      navigation.replace("HomeScreen");
      log("  Navigation complete to HomeScreen");
    }, 1000);
  };
  ...
  const handleNavigationStateChange = (navState) => {
    ...
    //    FALLBACK INTERCEPT: Success/Failure checks if onShouldStartLoadWithRequest missed it
    if (url.includes("/payu/success-forward") || url.includes("/payment-success")) {
      log("  SUCCESS forward URL detected in nav state - intercepting");
      if (!isRedirecting) {
    ...
    if (url.includes("/payu/failure-forward") || url.includes("/payment-failure")) {
      warn("  FAILURE forward URL detected in nav state - intercepting");
      if (!isRedirecting) {
        webViewRef.current?.stopLoading?.();
